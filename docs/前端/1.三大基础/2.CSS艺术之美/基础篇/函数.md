...
随着技术的不断进步，CSS 已经从简单的样式表发展成为拥有众多内置函数的强大工具。这些函数不仅增强了开发者的设计能力，还使得样式应用更加动态、灵活和响应式。本文将深入探讨 CSS 提供的 66 个函数，逐一剖析它们的功能和用法，一起进入这个充满魔力的 CSS 函数世界！

![1707675356763-9d03c4dc-b823-486b-9bb9-7772bc2a9f24.png](img/bk92jCYhwNkpFfl9/1707675356763-9d03c4dc-b823-486b-9bb9-7772bc2a9f24-070305.png)
...
`color-mix()` 是 CSS Color Module Level 5 提案中引入的一个实验性功能，用于混合两种颜色。该函数允许指定颜色空间（如 srgb、lch、lab 等）以及每种颜色混合的比例。
...
<code><font style="color:rgb(5, 7, 59);">rem()</font></code><font style="color:rgb(5, 7, 59);">函数类似于 JavaScript 的</font><code><font style="color:rgb(5, 7, 59);">%</font></code><font style="color:rgb(5, 7, 59);">操作符，用于计算除法操作的余数。它接受两个参数：被除数和除数，这两个参数必须是相同类型（数字、尺寸或百分比）的有效值。尽管参数的单位可以不同，但它们必须属于同一尺寸类型，如长度、角度、时间或频率。</font>
...
<code><font style="color:rgb(5, 7, 59);">rem()</font></code><font style="color:rgb(5, 7, 59);">函数返回的是一个与被除数符号相同的余数。例如，</font><code><font style="color:rgb(5, 7, 59);">rem(27, 5)</font></code><font style="color:rgb(5, 7, 59);">返回2，因为27除以5的商是5，余数是2。如果除数为0或被除数为无穷大，</font><code><font style="color:rgb(5, 7, 59);">rem()</font></code><font style="color:rgb(5, 7, 59);">函数将返回NaN（非数字）。此外，余数的符号始终与被除数的符号相同，即如果被除数为正，则余数为正；如果被除数为负，则余数为负。</font>
...
> 更新: 2024-02-24 16:26:59  
> 原文: <https://www.yuque.com/cuggz/feplus/ng2e90ni6atyknfv>