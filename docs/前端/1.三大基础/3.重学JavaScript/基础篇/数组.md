...
* **new Array(length)**：当 length 不是数值时，返回一个只包含 length 元素一项的数组；当 length 为数值时，length 最大不能超过 32 位无符号整型，即需要小于 2<sup>32</sup>，否则将抛出 RangeError。
...
#### <font style="color:#262626;background-color:transparent;">（3）ES6 构造器</font>

鉴于数组的常用性，ES6 专门扩展了数组构造器 Array ，新增了 2 个方法：Array.of和Array.from。Array.of 用得比较少，Array.from 具有很强的灵活性。

**<font style="color:#333333;"></font>**

\*\*<font style="color:#333333;">1）</font>\*\***Array.of**
...
![1611730183856-26a3b305-e13f-420c-ae0b-6e2befdd325f.png](img/kElmeuFc7MXsFfs8/1611730183856-26a3b305-e13f-420c-ae0b-6e2befdd325f-045736.png)
...
![1628425506519-3de80030-b418-4062-bed8-7a40f86ac2d1.png](img/kElmeuFc7MXsFfs8/1628425506519-3de80030-b418-4062-bed8-7a40f86ac2d1-675523.png)
...
* *value*\_**：**\_必需。填充的值；
* \_start：\_可选。开始填充位置；
* \_end：\_可选。停止填充位置 (默认为 *array*.length)。
...
* \_target'\_\_**：**\_必需。<font style="color:#333333;">复制到指定目标索引位置</font>；
* \_start：\_可选。<font style="color:#333333;">元素复制的起始位置</font>；
* \_end：\_可选。<font style="color:#333333;">停止复制的索引位置 (默认为 </font>*array*<font style="color:#333333;">.length)。如果为负值，表示倒数</font>。
...
其中参数<font style="color:#000000;">separator是可选的，用来</font><font style="color:#000000;">指定要使用的分隔符。如果省略该参数，则使用逗号作为分隔符。</font>

<font style="color:#000000;"></font>

<font style="color:#000000;">该</font><font style="color:#000000;">方法返回一个字符串。该字符串是通过把 arrayObject 的每个元素转换为字符串，然后把这些字符串连接起来，在两个元素之间插入 separator 字符串而生成的。</font>

<font style="color:#000000;"></font>

<font style="color:#000000;">使用示例如下：</font>
...
<font style="color:#000000;background-color:transparent;">pop() 方法用于删除并返回数组的最后一个元素。它没有参数。</font>\*\*该方法会改变原数组。\*\*其语法形式如下：
...
注意：<font style="color:#000000;background-color:transparent;">如果数组是空的，那么 shift() 方法将不进行任何操作，返回 undefined 值。</font>

#### （2）unshift()

unshift()<font style="color:#000000;background-color:transparent;">方法可向数组的开头添加一个或更多元素，并返回新的长度。</font>\*\*该方法会改变原数组。\*\*其语法形式如下：
...
其中参数sortby是<font style="color:#000000;">可选参数，用来规定排序顺序，它是一个比较函数，用来判断哪个值应该排在前面。默认情况下，sort()方法会按照升序重新排列数组元素。为此，sort()方法会在每一个元素上调用String转型函数，然后比较字符串来决定顺序，即使数组的元素都是数值，也会将数组元素先转化为字符串在进行比较、排序。这就造成了排序不准确的情况，如下代码：</font>
...
<font style="color:#000000;background-color:transparent;">reverse() 方法用于颠倒数组中元素的顺序</font><font style="color:#000000;">。该方法会改变原来的数组，而不会创建新的数组。其使用语法如下：</font>
...
其中参数arrayX是<font style="color:#000000;">必需的。该参数可以是具体的值，也可以是数组对象。可以是任意多个。</font>
...
* <font style="color:#000000;">index：必需。整数，规定添加/删除项目的位置，使用负数可从数组结尾处规定位置。</font>
* <font style="color:#000000;">howmany：</font><font style="color:#000000;background-color:transparent;">必需。要删除的项目数量。如果设置为 0，则不会删除项目。</font>
* <font style="color:#000000;">item1, ..., itemX：</font><font style="color:#000000;">可选。向数组添加的新项目。</font>

<font style="color:#000000;"></font>

<font style="color:#000000;">从上面参数可知，splice主要有三种使用形式：</font>
...
#### <font style="color:#333333;">（1）</font>arguments
...
![1612679309803-e5208c79-c41f-4dce-87a7-6b8f16f5afc8.png](img/kElmeuFc7MXsFfs8/1612679309803-e5208c79-c41f-4dce-87a7-6b8f16f5afc8-476653.png)
...
<font style="color:#404952;">可以看出，输出的就是函数自身，如果在函数内部直接执行调用 callee，那它就会不停地执行当前函数，直到执行到内存溢出。</font>
...
![1612679850622-ec390549-4b7a-49d2-ab58-45156d86d33c.png](img/kElmeuFc7MXsFfs8/1612679850622-ec390549-4b7a-49d2-ab58-45156d86d33c-451359.png)
...
![1612680061951-5c2803a4-8d30-49c9-b50f-a1147d4d4274.png](img/kElmeuFc7MXsFfs8/1612680061951-5c2803a4-8d30-49c9-b50f-a1147d4d4274-098488.png)
...
#### <font style="color:#262626;background-color:transparent;">（5）ES6 中的 flat</font>
...
<font style="color:#333333;">去除</font><font style="color:#333333;">无序数组中的重复元素并且返回新的无重复数组。</font>
...
> 更新: 2023-03-11 22:50:22  
> 原文: <https://www.yuque.com/cuggz/feplus/cp4k8o>