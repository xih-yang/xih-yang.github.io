...
不得不说，React 生态系统实在是太丰富了，仅状态管理这一领域就提供了诸多选项，且持续有新工具涌现。当前，React 状态管理的主流库包括 Redux、Zustand、MobX、Recoil、Jotai 等。其中，Redux 以遥遥领先的下载量独占鳌头，不过其增长速度有所放缓；而 Zustand 则紧随其后，尤其在近两年展现出迅猛的增长势头，成为不可忽视的力量。

![1736424357950-31b30a48-3b75-44d5-9adf-a091a28c1446.png](img/UqsWKuZduxeeTXdl/1736424357950-31b30a48-3b75-44d5-9adf-a091a28c1446-988811.png)
...
![1736415440189-360c93c2-7468-4e3d-9ed1-fbe1987f525b.png](img/UqsWKuZduxeeTXdl/1736415440189-360c93c2-7468-4e3d-9ed1-fbe1987f525b-693975.png)
...
Zustand 是一个轻量级状态管理库，专为 React 应用程序设计。它提供了简单直观的 API，旨在减少样板代码，并且易于集成和使用。其特点如下：
...
1. **创建 Store**：在 Zustand 中，Store是通过`create`函数创建的。每个Store都包含状态和处理状态的函数。
...
3. 订阅特定状态片段如果有一个包含多个状态的store，但在组件中只需要订阅其中一个状态，可以通过解构赋值从`useStore`返回的完整状态对象中提取需要的状态。Zustand的智能选择器功能允许这样做，而不会导致不必要的重新渲染。
...
1. \*\*创建 Store：\*\*Store 是存储应用程序状态的地方，并提供修改这些状态的方法。MobX 提供了多种方式来定义可观察的状态和操作这些状态的动作。
...
import counterStore from './CounterStore'; // 确保路径正确
...
reaction(
  () => counterStore.count, // 跟踪的依赖
  (count) => console.log(`Count changed to ${count}`) // 当依赖变化时触发的回调
);
```
...
```
- 在组件中使用 `useRecoilValue` Hook 仅获取原子或选择器的值（适用于只读场景）<font style="color:rgba(0, 0, 0, 0.85);">：</font>
```
...
1. **<font style="color:rgb(44, 44, 54);">创建 Atom</font>**<font style="color:rgb(44, 44, 54);">：</font>
   * **<font style="color:rgb(44, 44, 54);">简单原子创建</font>**<font style="color:rgb(44, 44, 54);">：定义一个 atom 来表示应用程序中的某个状态片段。</font>
...
> 更新: 2025-01-23 23:00:50  
> 原文: <https://www.yuque.com/cuggz/feplus/vxfc5x385a2tawqa>