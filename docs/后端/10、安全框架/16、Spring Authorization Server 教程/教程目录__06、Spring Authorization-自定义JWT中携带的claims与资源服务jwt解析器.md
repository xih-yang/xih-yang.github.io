...
- 来源：https://ddkk.com/zhuanlan/security/springauthorization/6.html
- 分类：安全框架
- 分组：教程目录
## 前言
...
生成一个access token，在[JSON Web Tokens (JWT) 在线解密](https://www.box3.cn/tools/jwt.html)中解析token，查看token内容
...
通过断点可以看到之前的自定token内容与自定jwt解析都已经生效了，已经将权限信息解析到当前用户信息的authorities中了。

补一张用户信息的图片

通过对比可以更清楚的观察到前文作出的自定义配置已经生效。

至于设置的角色在权限中有一个 **ROLE_** 的前缀是因为roles的方法实际上是调用authorities方法，并且自动加上该前缀，如上图所示，这种情况在自己实现**loadUserByUsername**时避免这种情况。
