...
- 来源：https://ddkk.com/zhuanlan/security/springauthorization/7.html
- 分类：安全框架
- 分组：教程目录
## 前言
...
修改登录接口，获取具体的异常信息，给页面一个友好的提示，如果接口不提供具体的异常则用户无法知道到底出了什么问题。

在上边的文档中有说明，如果登录失败会由`AuthorizationFailureHandler`处理，前文也提到过默认的是`SimpleUrlAuthenticationFailureHandler`；在`UsernamePasswordAuthenticationFilter`父类`AbstractAuthenticationProcessingFilter`中也有体现
