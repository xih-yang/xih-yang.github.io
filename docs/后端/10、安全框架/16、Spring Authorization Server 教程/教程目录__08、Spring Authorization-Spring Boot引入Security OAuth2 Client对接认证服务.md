...
- 来源：https://ddkk.com/zhuanlan/security/springauthorization/8.html
- 分类：安全框架
- 分组：教程目录
## 前言
      ...
      client:
        provider:
          ...
          custom-issuer:
            ...
            user-info-uri: ${
     spring.security.oauth2.client.provider.custom-issuer.issuer-uri}/user
        registration:
...
项目在初始化时会根据配置的issuer-uri拼接url，即签发地址根目录/.well-known/openid-configuration，像项目中配置的就是[http://192.168.120.33:8080/.well-known/openid-configuration](http://192.168.120.33:8080/.well-known/openid-configuration) ；该接口会返回认证服务器的元信息，如下：

所以通过该接口就可以很清楚的知道各授权端点的请求路径。

#### 客户端注册信息

默认是基于内存的，基本上来说服务注册的客户端不会很多，基于内存也就够用了；框架也提供了基于db的方式，需要自己注入一个JdbcOAuth2AuthorizedClientService；让客户端认证信息持久化。[文档](https://docs.spring.io/spring-security/reference/servlet/oauth2/login/core.html#oauth2login-register-clientregistrationrepository-bean)
...
**代码已提交至Gitee：[https://gitee.com/vains-Sofia/authorization-example](https://gitee.com/vains-Sofia/authorization-example)**
