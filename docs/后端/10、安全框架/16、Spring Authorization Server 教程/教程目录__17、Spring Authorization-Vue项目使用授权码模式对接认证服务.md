...
- 来源：https://ddkk.com/zhuanlan/security/springauthorization/17.html
- 分类：安全框架
- 分组：教程目录
...
**5、** 客户端通过查询参数获取`authorization_code`并向[TokenEndpoint][]发起请求；
...
export function getToken(data: any) {
    ...
    if (data.client_secret) {
       ...
       base64Str(${ data.client_id}:${ data.client_secret})}
        // 移除入参中的key
...
import {
      generateCodeVerifier } from '@/util/pkce'
import {
...
if (code) {
  ...
  window.location.href = ${
       import.meta.env.VITE_OAUTH_ISSUER}/oauth2/authorize?client_id=${
