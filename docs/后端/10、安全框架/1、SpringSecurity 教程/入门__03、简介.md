# 03、简介
- 来源：https://ddkk.com/zhuanlan/security/springsecurity/1/3.html
- 分类：安全框架
- 分组：入门
## Spring Security是什么?
...
规避这个问题的一种方式是在你的pom文件的``部分包含所有的Spring框架模块。另一种方法是像下面这样在`pom.xml`中包含`spring-framework-bom`:
...
.springframework.security.provisioning
```
...
```java
clone https://github.com/spring-projects/spring-security.git
```
