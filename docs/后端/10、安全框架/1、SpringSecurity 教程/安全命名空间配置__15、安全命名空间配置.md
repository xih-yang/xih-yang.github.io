# 15、安全命名空间配置
- 来源：https://ddkk.com/zhuanlan/security/springsecurity/1/15.html
- 分类：安全框架
- 分组：安全命名空间配置
## 简介
...
这比配置一个Apache目录服务器bean简单得多。最常见的替代配置需求是 `ldap-server`元素的属性支持，用户不用担心他们要创建的bean的属性名称（注：你可以从Chapter 26, LDAP 身份验证找到如何使用`ldap-server` 元素的更多信息 ）。当编辑应用的Context文件时良好的XML编辑器应该可以提供可用的属性和元素的信息。我们建议你尝试使用[SpringSource Tool Suite](http://www.springsource.com/products/sts)，它具有与标准Spring 命名空间工作的特别的功能。
