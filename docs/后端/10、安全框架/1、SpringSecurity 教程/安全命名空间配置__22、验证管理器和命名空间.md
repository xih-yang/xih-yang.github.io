# 22、验证管理器和命名空间
- 来源：https://ddkk.com/zhuanlan/security/springsecurity/1/22.html
- 分类：安全框架
- 分组：安全命名空间配置
...
您可能需要使用`ProviderManager`注册其他的`AuthenticationProvider` Bean，你可以使用``元素的`ref`属性，在属性的值是您要添加的bean的名字。例如：
