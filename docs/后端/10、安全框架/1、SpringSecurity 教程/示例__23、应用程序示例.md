# 23、应用程序示例
- 来源：https://ddkk.com/zhuanlan/security/springsecurity/1/23.html
- 分类：安全框架
- 分组：示例
...
该JAAS是很简单的如何用Spring Security使用JAAS LoginModule的例子。所提供的`LoginModule`将成功地验证用户，如果用户名和密码相等，否则抛出一个`LoginException`异常。在本实例中使用的的`AuthorityGranter`总是授予角色ROLE*USER。示例应用程序还演示了如何通过设置LoginModule的jaas-api-provision等于true返回JAAS Subject 来运行(***)。
