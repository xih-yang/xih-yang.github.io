# 05、Shiro InI 配置
- 来源：https://ddkk.com/zhuanlan/security/shiro/1/5.html
- 分类：安全框架
- 分组：教程目录
## INI 配置
...
**1、** 对象名=全限定类名相对于调用public无参构造器创建对象；

**2、** 对象名.属性名=值相当于调用setter方法设置常量值；

**3、** 对象名.属性名= `$` 对象引用相当于调用setter方法设置对象引用；

**3、** Java代码（com.github.zhangkaitao.shiro.chapter4.ConfigurationCreateTest）；
...
ini配置文件类似于 Java 中的 properties（key=value），不过提供了将 key/value 分类的特性，key 是每个部分不重复即可，而不是整个配置文件。如下是 INI 配置分类：
...
配置url 及相应的拦截器之间的关系，格式：“url = 拦截器 [参数]，拦截器 [参数]，如：
