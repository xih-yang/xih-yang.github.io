...
- 来源：https://ddkk.com/zhuanlan/mq/kafka/2/22.html
- 分类：消息队列
- 分组：教程目录
## 写在前面的话
...
上述幂等设计只能保证单个Producer对于同一个``的`Exactly Once`语义。
...
**1、** 将`PREPARE_COMMIT`或`PREPARE_ABORT`消息写入`TransactionLog`，如上图中步骤5.1所示；

**2、** 通过`WriteTxnMarker`请求以`TransactionMarker`的形式将`COMMIT`或`ABORT`信息写入用户数据日志以及`OffsetLog`中，如上图中步骤5.2所示；

**3、** 最后将`COMPLETE_COMMIT`或`COMPLETE_ABORT`信息写入`TransactionLog`中，如上图中步骤5.3所示；
