...
- 来源：https://ddkk.com/zhuanlan/mq/kafka/8/2.html
- 分类：消息队列
- 分组：教程目录
...
- 如果Producer使用同步模式则Producer会在尝试重新发送message.send.max.retries（默认值为3）次后抛出Exception，用户可以选择继续选择发送后续数据也可选择停止发送。而前者会造成数据的阻塞，后者会造成本应发往该Broker的数据的丢失；
...
**3、** 将第i个Partition的第j个Replica分配到第（(i+j)modn）个Broker上；
