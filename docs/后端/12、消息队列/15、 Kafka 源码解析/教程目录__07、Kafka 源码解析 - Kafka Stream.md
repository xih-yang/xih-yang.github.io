...
- 来源：https://ddkk.com/zhuanlan/mq/kafka/8/7.html
- 分类：消息队列
- 分组：教程目录
...
> 注意：Kafka Stream的并行模型，非常依赖于《Kafka 源码分析（一）- Kafka背景及架构介绍》一文中介绍的Kafka分区机制和《Kafka 源码分析（四）- Kafka Consumer设计解析》中介绍的Consumer的Rebalance机制。强烈建议不太熟悉这两种机制的朋友，先行阅读这两篇文章。
