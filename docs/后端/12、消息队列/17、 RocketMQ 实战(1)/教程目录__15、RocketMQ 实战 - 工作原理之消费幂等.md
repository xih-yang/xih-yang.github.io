...
- 来源：https://ddkk.com/zhuanlan/mq/rocketmq/5/15.html
- 分类：消息队列
- 分组：教程目录
## 1 什么是消费幂等
...
**1、** 首先通过缓存去重在缓存中如果已经存在了某幂等令牌，则说明本次操作是重复性操作；若缓存没有命中，则进入下一步；

**2、** 在唯一性处理之前，先在数据库中查询幂等令牌作为索引的数据是否存在若存在，则说明本次操作为重复性操作；若不存在，则进入下一步；
...
**1、** 当支付请求到达后，首先在Redis缓存中却获取key为支付流水号的缓存value若value不空，则说明本次支付是重复操作，业务系统直接返回调用侧重复支付标识；若value为空，则进入下一步操作；

**2、** 到DBMS中根据支付流水号查询是否存在相应实例若存在，则说明本次支付是重复操作，业务系统直接返回调用侧重复支付标识；若不存在，则说明本次操作是首次操作，进入下一步完成唯一性处理；
