...
- 来源：https://ddkk.com/zhuanlan/mq/rocketmq/6/1.html
- 分类：消息队列
- 分组：教程目录
### 一、为什么选择RocketMQ
...
支持，需要ZooKeeper服务器
不支持
Kafka使用键值对格式进行配置。这些值可以从文件中提供，也可以通过编程方式提供。
支持，使用终端命令公开核心度量

RocketMQ
Java, C++, Go
拉取模型，支持TCP、JMS、OpenMessaging
确保消息的严格排序，并且可以优雅地扩展
支持
支持，具有同步模式以避免消息丢失
支持
