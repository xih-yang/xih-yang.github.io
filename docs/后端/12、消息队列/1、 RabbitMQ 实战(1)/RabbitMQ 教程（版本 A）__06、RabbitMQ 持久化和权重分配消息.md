...
- 来源：https://ddkk.com/zhuanlan/mq/rabbitmq/1/6.html
- 分类：消息队列
- 分组：RabbitMQ 教程（版本 A）
## 1、RabbitMQ 持久化简介
...
**为了避免这种情况，我们可以在消费者中 设置参数 channel.basicQos(1)**

**启动消费者**

**在提供者中输入以下内容**

**分别查看两个消费者,发现在30S内输入内容延迟1S的工作线程接收消息多，而延迟30S的工作线程只接收一条消息**
