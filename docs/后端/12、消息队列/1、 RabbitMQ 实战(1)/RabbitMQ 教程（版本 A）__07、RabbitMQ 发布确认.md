# 07、RabbitMQ 发布确认
- 来源：https://ddkk.com/zhuanlan/mq/rabbitmq/1/7.html
- 分类：消息队列
- 分组：RabbitMQ 教程（版本 A）
## 1、发布确认原理
...
发布确认默认是没有开启的，如果要在发布者中开启需要调用方法 confirmSelect，每当你要想使用发布确认，都需要在 channel 上调用该方法
...
```java
package com.zww.spring.rabbitmq.four;
    ...
    public static void main(String[] args) throws Exception {
        ...
        * */
        ...
        ConfirmCallback confirmCallback = (deliveryTag, multiple) -> {
            //二、批量删除掉已经确认的消息 剩下就是未确认消息
            if (multiple){
                ConcurrentNavigableMap<Long, String> confirmed = outstandingConfirms.headMap(deliveryTag);
                confirmed.clear();//清理已经确认的消息
            }else {
                //单个删除已确认的消息
        ...
        * */
        ...
        for (int i = 0; i < MESSAGE_COUNT; i++) {
            ...
            //处理异步未确认消息
            // 一、记录所有要发送的消息 消息的总和
            //channel.getNextPublishSeqNo()：消息序号
