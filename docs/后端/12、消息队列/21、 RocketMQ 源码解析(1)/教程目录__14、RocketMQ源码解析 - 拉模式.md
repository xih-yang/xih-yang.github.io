# 14、RocketMQ源码解析 - 拉模式
- 来源：https://ddkk.com/zhuanlan/mq/rocketmq/1/14.html
- 分类：消息队列
- 分组：教程目录
## 1、RocketMQ 推拉模式简介
...
PullRequest 是由 RebalanceService 产生，它根据主题消息队列个数和当前消费组内消费者个数进行负载，然后产生对应的 PullRequest 对象，再将这些对象放入到 PullMessageService 的 pullRequestQueue队列。具体放入逻辑调用：RebalanceImpl#dispatchPullRequest(final List ``pullRequestList);
