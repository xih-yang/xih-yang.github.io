...
- 来源：https://ddkk.com/zhuanlan/mq/rocketmq/4/4.html
- 分类：消息队列
- 分组：教程目录
**1、** Broker发送心跳注册请求源码；
...
Broker启动过程中, 会跟所有的NameServer建立和保持长连接, 然后开启定时任务发送心跳包, 心跳包中含有当前Broker的信息, 包括地址、名称、id等, 以及存储topic的信息。 注册后, NameServer就有Topic和Broker的映射关系。
...
最后启动了一个定时任务, 默认情况下每隔30s调用registerBrokerAll方法向所有的nameServer进行一次注册broker信息, 时间间隔可以配置在registerNameServerPeriod属性, 这个定时任务就是Broker向nameserver发送的心跳包的定时任务, 包括topic名、读、写队列个数、队列权限、是否有序等信息。

#### 1.2 registerBrokerAll注册broker信息

registerBrokerAll方法用于当前Broker将自身信息注册到所有的NameServer中。

内部调用的doRegisterBrokerAll方法执行注册, 先判断是否需要注册, 如果如果如果forceRegister为true, 那么表示强制注册, 如果当前broker应该注册, 向nameServer进行注册。
...
- needRegister是否需要注册: 判断当前broker是否需要向nameserver进行注册, 当forceRegister参数为true的时候, 表示强制注册, 如果forceRegister为faslse的话, 那么borker是否需要向nameserver注册就得看这个方法的结果了。
...
向所有nameServer发起请求, 请求code为QUERY_DATA_VERSION, 获取所有nameserver的DataVersion数据, 判断DataVersion数据与自身的是否一致, 如果不一致, 重新注册。
...
DataVersion是RocketMQ的数据版本控制机制, Dataversion和topic的配置都被持久化到topics.json文件中, DataVersion和topic的配置被加载到内存之后, 分别会解析成为topicConfigManager的topicConfigTablehe属性和dataVersion属性。

- doRegisterBrokerAll注册broker信息: 向所有nameServer发起请求。
...
内部调用BrokerOuterAPI#registerBrokerAll方法, BrokerOuterAPI类是为broker向外部发起请求的api方法。

broker要向所有的nameServer进行注册, 需要提高性能, registerBrokerAll方法里面使用了多线程机制, 使用了brokerOuterExecutor线程池并行的发起对于每个nameserver的注册请求。需要保证线程安全和控制并发。

**1、** 需要在多线程中将执行结果并行存入集合中,RocketMQ使用了CopyOnWriteArrayList保证线程安全；

**2、** 主线程需要等待这些请求都执行完毕,RocketMQ使用了CountDownLatch倒计数器,可以使得主线程阻塞,确保在其他线程任务执行完毕之后,再唤醒主线程；
...
registerBroker注册broker

线程任务是调用另一个registerBroker方法, 该方法真正的执行向一个nameserver发起注册的请求。

broker注册请求为同步请求, 注册的信息主要包括自身的所有topic数据, dataVersion, filterServerList, 以及包括集群名、broker地址、broker名、brokerId等等在内的一些broker自身的信息。
...
Broker发送心跳包后, nameserver会进行专门的处理, 保存或者更新broker上报的心跳包数据。

#### 2.1 处理心跳包入口

NameServer的默认网络处理器是DefaultRequestProcessor, 故心跳请求的入口也就是DefaultRequestProcessor#processRequest方法。

processRequest方法是一个通用的请求处理入口方法, 根据请求的不同requestCode进入分发处理, 心跳请求的requestCode为REGISTER_BROKER 103。
...
nameserver会调用registerBrokerWithFilterServer方法来处理大于3.0.11版本的broker的注册请求。

该方法会解析请求头、请求体中的内容，内容包括topic信息、版本信息dataVersion、消息过滤信息filterServerList、以及broker基本信息，例如broker地址、名字等等。最后会调用routeInfoManager#registerBroker方法实现broker信息的注册。
...
NameServer通过routeInfoManager#registerBroker方法实现broker信息的注册。

##### 2.2.1 RouteInfoManager介绍#

RouteInfoManager这个类位于namesrv模块中, 用于管理nameServer上的关于真个RocketMQ集群的各种路由信息。
...
##### 2.2.2 registerBroker注册broker#

对broker的各种路由信息进行更新或者注册。
...
NameServer启动的时候, 会启动一个定时周期任务, 默认每隔10s执行一次扫描无效Broker, 并清除Broker相关路由信息的任务。
...
扫描RouteInfoManager的brokerLiveTable集合中的每一个BrokerLiveInfo, 判断如果当前时间戳 大于 上次接收心跳的时间戳 + Broker过期时间, 那么剔除Broker, Broker的过期时间默认为120s。

剔除:

1. 调用RemotingUtil#closeChannel关闭和当前broker的通道, 关闭broker的socket连接。
...
onChannelDestroy清除路由信息: 断开连接之后请求与该broker相关的无效路由信息。

删除信息的时候同样需要先加写锁, 然后从brokerLiveTable、filterServerTable、brokerAddrTable、clusterAddrTable、topicQueueTable这五个路由表中删除数据。
...
如果broker和nameserver之间的长连接异常关闭, 那么此前绑定的BrokerHousekeepingServer发挥了作用。

BrokerHousekeepingService继承了ChannelEventListener, 当触发连接异常事件时, BrokerHousekeepingServer内部的方法会调用RouteInfoManager#onChannelDestroy清除路由信息。
...
- broker的信息会向nameServer集群中的每一个节点都上报数据, 即心跳包, 数据包含broker的基本信息, 例如brokerAddr、brokerId、brokerName、clusterName等, 以及broker的topic配置信息, 比如topicName名字、perm权限、读写队列数量等等属性, 当前上报的数据的DataVersion, 以及消费过滤信息集合filterServerList。
...
- 每个nameServer之间不会互相通信, 数据不会同步, NameServer的所有路由数据都存储在内存中, 没有持久化。nameServer是轻量级的。
- nameServer没有数据同步、持久化等机制, 可能数据不一致, 但是保证服务高可用。保证了CAP中的AP。
