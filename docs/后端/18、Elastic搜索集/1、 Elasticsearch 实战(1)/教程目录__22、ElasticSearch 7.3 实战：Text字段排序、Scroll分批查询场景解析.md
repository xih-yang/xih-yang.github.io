...
- 来源：https://ddkk.com/zhuanlan/search/elasticsearch/1/22.html
- 分类：搜索引擎
- 分组：教程目录
## 1、Text字段排序
...
**2、** mapping时设置`fielddata:true`，按照第一个分词进行字典序排序这种方式也不是很准确，因为只是按照第一个分词进行排序，后续的分词不会参与排序fielddata:true的排序是对text内部分词结果进行排序之后再进行外部排序的,效率低不推荐使用;；
