# 10、Minio 教程 - Minio 之权限控制策略
- 来源：https://ddkk.com/zhuanlan/filestorage/minio/1/10.html
- 分类：分布式存储
- 分组：教程目录
### 前言
...
Statement
策略授权语句，描述策略的详细信息，包含Effect（效果）、Action（动作）、Principal（用户）、Resource（资源）和Condition（条件）。其中Condition为可选
