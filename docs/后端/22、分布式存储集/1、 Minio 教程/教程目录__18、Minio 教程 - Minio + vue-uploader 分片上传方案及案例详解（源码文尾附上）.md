# 18、Minio 教程 - Minio + vue-uploader 分片上传方案及案例详解（源码文尾附上）
- 来源：https://ddkk.com/zhuanlan/filestorage/minio/1/18.html
- 分类：分布式存储
- 分组：教程目录
### 前言
...
**2、** 前端服务直接请求Minio服务器，并发上传分片；

**3、** 所有分片上传完成后，使用uploadId调用文件服务进行文件合并；
...
首先自定义一个Minio 客户端类，继承`MinioClient`类，其作用主要是将以下几个方法暴露出来，以便调用：
...
在模板类`MinioTemplate`中，将之前的MinIO 客户端换成`PearlMinioClient`。并将上面关于分片的几个操作方法集成进来。
...
最后附上[前后端代码地址](https://gitee.com/pearl88/study-demo/tree/master/spring-boot-minio)。
