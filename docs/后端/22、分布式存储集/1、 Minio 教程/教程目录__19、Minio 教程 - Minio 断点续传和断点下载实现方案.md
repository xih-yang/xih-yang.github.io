# 19、Minio 教程 - Minio 断点续传和断点下载实现方案
- 来源：https://ddkk.com/zhuanlan/filestorage/minio/1/19.html
- 分类：分布式存储
- 分组：教程目录
### 前言
...
要实现断点续传，需要依赖于MD5秒传和分片上传。之前我们分片上传的简单流程如下图所示，我们可以在这个流程中加入端点续传。
...
在Minio中的java SDK 中是提供了对象数据查询的[API](https://blog.csdn.net/qq_43437874/article/details/120849494)，可以查看到对象的数据大小信息。

知道文件大小之后，我们就可以进行切片，比如100M的大小，按照5M进行切分为20片，计算出每一片的起始位置和长度，Minio 获取文件的API 是支持分段获取流的。

在实现了上述的分片下载后，就可以实现断点下载了，原理和上传一样，如下图所示：

关于前端下载文件的类似框架貌似找不到，那么只能自己实现了。。。。
