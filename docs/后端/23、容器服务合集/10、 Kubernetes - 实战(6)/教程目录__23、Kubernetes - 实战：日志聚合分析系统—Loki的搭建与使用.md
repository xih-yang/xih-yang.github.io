...
- 来源：https://ddkk.com/zhuanlan/container/kubernetes/6/23.html
- 分类：容器服务
- 分组：教程目录
## 日志聚合分析系统——Loki
...
**1、** Distributor接收到HTTP请求，用于存储流数据；
...
**2、** Querier将请求发送至Ingester读取还未写入Chunks的内存数据；

**3、** 随后再去index+chunks中查找数据；

**4、** Querier遍历所有数据并进行去重处理，再返回最终结果；
...
vimcharts/promtail/values.yaml
