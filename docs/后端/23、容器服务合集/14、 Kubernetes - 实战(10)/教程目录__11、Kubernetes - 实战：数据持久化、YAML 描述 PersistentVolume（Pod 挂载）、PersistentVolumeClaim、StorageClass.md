...
- 来源：https://ddkk.com/zhuanlan/container/kubernetes/10/11.html
- 分类：容器服务
- 分组：教程目录
...
现在我们就一起来了解 `Volume` 的高级用法，看看 `Kubernetes` 管理存储资源的 `API` 对象 `PersistentVolume`、`PersistentVolumeClaim`、`StorageClass`然后使用本地磁盘来创建实际可用的存储卷。
...
[K8S PV一直Released状态的解决办法](https://blog.csdn.net/wz_cmyc/article/details/120542934)

[k8s pv 一直是release状态](https://blog.csdn.net/koukouwuwu/article/details/117514801)
...
修改`PVC` 申请容量为 100Mi，`kubctl apply` 之后，再次查看 `PV` 和 `PVC` 状态。
