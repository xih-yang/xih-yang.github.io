...
- 来源：https://ddkk.com/zhuanlan/container/kubernetes/12/18.html
- 分类：容器服务
- 分组：教程目录
## Pod资源调度

APIServer在接受客户端提交Pod对象创建请求后，然后是通过调度器（kube-schedule）从集群中选择一个可用的最佳节点来创建并运行Pod。而这一个创建Pod对象，在调度的过程当中有3个阶段：节点预选、节点优选、节点选定，从而筛选出最佳的节点。如图：
...
Kubernetes使用**PodToleratesNodeTaints**预选策略和**TaintTolerationPriority**优选函数来完成这种调度方式。
...
Pod对象的容忍度可以通过**`spec.tolerations`**字段进行添加，同一的也有两种操作符：**`Equal`**和**`Exists`**方式。

容忍基于operator的匹配污点：

**如果operator是Exists，则容忍度不需要value而是直接匹配污点类型**

**如果operator是Equal，则需要指定value 并且value的值要等于tolerations的key**
