...
- 来源：https://ddkk.com/zhuanlan/container/docker/1/30.html
- 分类：容器服务
- 分组：教程目录
...
-hHOSTNAME or --hostname=HOSTNAME 设定容器的主机名，它会被写到容器内的 /etc/hostname 和 /etc/hosts。但它在容器外部看不到，既不会在 docker ps 中显示，也不会在其他的容器的 /etc/hosts 看到。
