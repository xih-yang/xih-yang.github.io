...
- 来源：https://ddkk.com/zhuanlan/container/docker/1/56.html
- 分类：容器服务
- 分组：教程目录
...
$ sudo ip Linkset A up
```

将B放到容器的网络命名空间，命名为 eth0，启动它并配置一个可用 IP（桥接网段）和默认网关。

```sh
$ sudo ip Linkset B netns $pid
$ sudo ip netns exec $pid ip Linkset dev B name eth0
$ sudo ip netns exec $pid ip Linkset eth0 up
