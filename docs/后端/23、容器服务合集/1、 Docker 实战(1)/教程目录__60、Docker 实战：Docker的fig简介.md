...
- 来源：https://ddkk.com/zhuanlan/container/docker/1/60.html
- 分类：容器服务
- 分组：教程目录
## 快速搭建基于 Docker 的隔离开发环境

使用Dockerfile 文件指定你的应用环境，让它能在任意地方复制使用：
...
在fig.yml 文件中指定应用使用的不同服务，让它们能够在一个独立的环境中一起运行：
...
在requirements.txt 文件中指定应用的 Python 依赖包。
...
这个web 应用已经开始在你的 docker 守护进程里面监听着 5000 端口了（如果你有使用 boot2docker ，执行 boot2docker ip ，就会看到它的地址）。
