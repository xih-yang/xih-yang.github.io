...
- 来源：https://ddkk.com/zhuanlan/container/docker/1/65.html
- 分类：容器服务
- 分组：教程目录
## 使用 Django 入门 Fig
...
Fig会先使用 Dockerfile 为 web 服务创建一个镜像，接着使用这个镜像在容器里运行 django-admin.py startproject figexample . 指令。
...
这个web 应用已经开始在你的 docker 守护进程里监听着 5000 端口了（如果你有使用 boot2docker ，执行 boot2docker ip ，就会看到它的地址）。
