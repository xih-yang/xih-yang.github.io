...
- 来源：https://ddkk.com/zhuanlan/container/docker/1/74.html
- 分类：容器服务
- 分组：教程目录
## WordPress
...
[WordPress](https://en.wikipedia.org/wiki/WordPress) 是开源的 Blog 和内容管理系统框架，它基于 PhP 和 MySQL。 该仓库提供了 WordPress 4.0 版本的镜像。
...
```sh
$ sudo docker run --name some-wordpress --Linksome-mysql:mysql -d wordpress
```

启动WordPress 容器时可以指定的一些环境参数包括
