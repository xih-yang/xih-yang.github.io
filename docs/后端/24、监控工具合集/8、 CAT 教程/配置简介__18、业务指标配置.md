# 18、业务指标配置
- 来源：https://ddkk.com/zhuanlan/linktrack/cat/18.html
- 分类：链路追踪
- 分组：配置简介
## 业务指标配置
...
与Transaction 不同，业务指标更偏向于一个应用在宏观上的指标。

业务指标由 logMetric() 上报而来，对应logMetric(String name, Map`` tags) 中的name。
...
**2、** 编辑标题(标题用于图表展示和告警)，顺序，是否开启告警，以及是否为敏感数据（敏感数据只显示趋势图，不显示具体值）:![ ][nbsp1]；
...
业务标签是由 logMetric() 上而来，对应业务指标对应logMetric(String name, Map`` tags) 中的 tags。
