# 04、Maven 构建生命周期
- 来源：https://ddkk.com/zhuanlan/tools/maven/4.html
- 分类：开发工具
- 分组：教程目录
## Maven – 构建生命周期
...
这里的clean 阶段将会被首先执行，然后 dependency:copy-dependencies 目标会被执行，最终 package 阶段被执行。
...
这是Maven 的主要生命周期，被用于构建应用。包括下面的 23 个阶段。
...
不同的maven 目标将根据打包的类型（JAR / WAR / EAR），被绑定到不同的 Maven 生命周期阶段。
