# 09、Gradle Web 工程构建
- 来源：https://ddkk.com/zhuanlan/tools/gradle/9.html
- 分类：开发工具
- 分组：教程目录
## Web 工程构建
...
要启动Web 工程，只需使用 Jetty plugin 即可：
...
由于Jetty plugin 继承自 War plugin。调用 gradle jettyRun 将会把你的工程启动部署到 jetty 容器中。调用 gradle jettyRunWar 会打包并启动部署到 jetty 容器中。
