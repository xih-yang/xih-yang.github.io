...
- 来源：https://ddkk.com/zhuanlan/server/netty/3/6.html
- 分类：服务器框架
- 分组：教程目录
...
本文首先展示了`ByteBuffer`在**写入模式**和**读取模式**下内部的一个状态，然后分析了`clear()` 与 `compact()` 方法的源码，最后讲解了`ByteBuffer`的使用案例。下节我们再来分析`ByteBuf`实现原理。
