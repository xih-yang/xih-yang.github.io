...
- 来源：https://ddkk.com/zhuanlan/server/netty/3/15.html
- 分类：服务器框架
- 分组：教程目录
...
注意添加到`ChannelPipeline`的`ChannelHandler`的顺序，MyLineBasedFrameDecoder 在前，MyLineBasedFrameDecoderServerHandler 在后，意味着数据先经过`MyLineBasedFrameDecoder`解码，然后再交给`MyLineBasedFrameDecoderServerHandler`处理。
