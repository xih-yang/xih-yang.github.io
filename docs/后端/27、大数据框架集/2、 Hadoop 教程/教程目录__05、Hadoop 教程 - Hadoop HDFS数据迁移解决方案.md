...
- 来源：https://ddkk.com/zhuanlan/bigdata/hadoop/2/5.html
- 分类：大数据框架
- 分组：教程目录
## 1. HDFS数据迁移解决方案
...
在HDFS 中，同样有许多需要数据迁移的场景，比如冷热数据集群之间的数据转化， 或者 HDFS 数据的双机房备份等等 。因为涉及跨机房 、跨集群，所以数据迁移不会是一个简单的操作。
...
上面的命令表示从`nn1`集群拷贝`/foo/a`路径下的数据到`nn2`集群的`/bar/foo`路径下。
