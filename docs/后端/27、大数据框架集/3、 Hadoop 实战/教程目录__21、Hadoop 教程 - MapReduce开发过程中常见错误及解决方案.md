# 21、Hadoop 教程 - MapReduce开发过程中常见错误及解决方案
- 来源：https://ddkk.com/zhuanlan/bigdata/hadoop/3/21.html
- 分类：大数据框架
- 分组：教程目录
## 1. 导包出错

在导入包的时候容易出错。尤其Text和CombineTextInputFormat。
...
如果报如上异常“ java.lang.Exception: java.io.IOException: Illegal partition for 13926435656 (4) ”，那说明Partition和ReduceTask个数没对上，调整ReduceTask个数。
...
在Windows环境编译的jar包导入到Linux环境中运行，hadoop jar wc.jar com.atguigu.mapreduce.wordcount.WordCountDriver /user/atguigu/ /user/atguigu/output
...
部分为路径书写错误。还有就是要检查后缀的问题（避免txt.txt）。还有个别电脑写相对路径找不到，可以修改为绝对路径。
...
解决方案一：拷贝hadoop.dll文件到Windows目录C:\Windows\System32。个别同学电脑还需要修改Hadoop源码。
