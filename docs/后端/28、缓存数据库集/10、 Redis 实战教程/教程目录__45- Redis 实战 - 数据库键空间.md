# 45- Redis 实战 - 数据库键空间
- 来源：https://ddkk.com/zhuanlan/db/redis-action/45.html
- 分类：缓存数据库
- 分组：教程目录
...
键book 以及它的值将从键空间中被删除， 如图 IMAGE_DB_AFTER_DEL 所示。
...
键message 的值对象将从之前包含 "hello world" 字符串更新为包含 "blah blah" 字符串， 如图 IMAGE_DB_UPDATE_CAUSE_SET 所示。
...
GET命令将首先在键空间中查找键 message ， 找到键之后接着取得该键所对应的字符串对象值， 之后再返回值对象所包含的字符串 "helloworld" ， 取值过程如图 IMAGE_FETCH_VALUE_VIA_GET 所示。
