# 46、MongoDB 正则表达式
- 来源：https://ddkk.com/zhuanlan/db/mongodb/46.html
- 分类：缓存数据库
- 分组：教程目录
...
以下命令使用正则表达式查找包含 souyunku 字符串的文章：

```sh
>db.posts.find({post_text:{$regex:"souyunku"}})
```

以上查询也可以写为：

```sh
>db.posts.find({post_text:/souyunku/})
```
...
```sh
>db.posts.find({post_text:{$regex:"souyunku",$options:"$i"}})
```
