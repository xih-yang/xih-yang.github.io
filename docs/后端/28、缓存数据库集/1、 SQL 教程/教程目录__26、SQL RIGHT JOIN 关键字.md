...
- 来源：https://ddkk.com/zhuanlan/db/sql/26.html
- 分类：缓存数据库
- 分组：教程目录
SQLRIGHT JOIN 关键字从右表 ( table2 )返回所有的行，即使左表 ( table1 )中没有匹配。如果左表中没有匹配，则结果为 NULL
...
可以看到每个表的最后一条记录是不匹配的，那么 RIGHT JOIN 的结果就是
...
假设我们要查看每天的访问量，那么可以使用下面的 SQL 语句

```sql
SELECT lession.id,lession.name,lession_views.date_at, lession_views.views FROM lession RIGHT JOIN lession_views ON lession.id=lession_views.lession_id ORDER BY lession.id DESC;
```

输出结果如下

```sql
mysql> SELECT lession.id,lession.name,lession_views.date_at, lession_views.views FROM lession RIGHT JOIN lession_views ON lession.id=lession_views.lession_id ORDER BY lession.id DESC;
+------+---------------------+----------+-------+
...
| NULL | NULL                | 20170512 |    12 |
| NULL | NULL                | 20170511 |    22 |
| NULL | NULL                | 20170513 |    39 |
+------+---------------------+----------+-------+
```
