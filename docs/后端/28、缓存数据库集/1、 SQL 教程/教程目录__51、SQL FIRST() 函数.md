# 51、SQL FIRST() 函数
- 来源：https://ddkk.com/zhuanlan/db/sql/51.html
- 分类：缓存数据库
- 分组：教程目录
SQL中的 FIRST() 函数返回指定的列中第一个记录的值
...
```sql
SELECT TOP 1 lession_name FROM lession_views ORDER BY views DESC;
```
...
```sql
SELECT lession_name FROM lession_views ORDER BY views DESC LIMIT 1;
```
...
```sql
SELECT lession_name FROM lession_vies ORDER BY views DESC WHERE ROWNUM <=1;
```
...
下面的SQL 语句读取 lession 表中浏览量最多的课程 ( lession_name )
...
```sql
mysql> SELECT name FROM lession ORDER BY views DESC LIMIT 1;
+---------------------+
| name                |
+---------------------+
