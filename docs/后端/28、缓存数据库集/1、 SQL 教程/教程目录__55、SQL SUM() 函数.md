# 55、SQL SUM() 函数
- 来源：https://ddkk.com/zhuanlan/db/sql/55.html
- 分类：缓存数据库
- 分组：教程目录
SQL中的 SUM() 函数返回数值列的总数
...
下面的SQL 语句统计全部课程的 **访问量 ( views )**

```sql
SELECT SUM(views) as total_views FROM lession;
```

运行结果输出如下

```sql
mysql> SELECT SUM(views) as total_views FROM lession;
+-------------+
| total_views |
+-------------+
|        1253 |
