...
- 来源：https://ddkk.com/zhuanlan/db/redis/4/27.html
- 分类：缓存数据库
- 分组：教程目录
## Redis 故障转移流程和原理
...
当一个节点出现问题，需要使用一种健壮的方法保证识别出节点是否发生了故障。在之前的 [Redis Cluster 通信流程深入剖析](/zhuanlan/db/redis/4/25.html) 一文中，介绍了`Redis`的`gossip`协议，集群节点通过`PING/PONG`消息实现节点通信，消息不但可以传播节点槽信息，还可以传播主从状态、节点故障信息等。因此故障检测也是就是通过消息传播机制实现的。
