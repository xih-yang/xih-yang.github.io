...
- 来源：https://ddkk.com/zhuanlan/xml/xpath/9.html
- 分类：XML语言教程
- 分组：教程目录
## 存取函数
...
结果,向外部处理环境返回 err:toohigh 的错误信息

```xml
http://example.com/test#toohigh "Error: Price is too high"
```
...
根据代码点序列返回字符串
例子：codepoints-to-string(84, 104, 233)
结果：'Thé'
...
fn:codepoint-equal(c1,c2)
根据 Unicode 代码点对照1
如果 c1 的值等于 c2 的值，则返回 true 否则返回 false

fn:compare(c1,c2)
fn:compare(c1,c2,collation)
如果 c1 小于 c2，则返回 -1
如果 c1 等于 c2，则返回 0
如果 c1 大于 c2，则返回 1
（根据所用的对照规则）
...
返回使用 sep 作为拼接符拼接 string 参数后的字符串
...
fn:string-length()
返回指定字符串的长度
如果没有 string 参数，则返回当前节点的字符串值的长度
...
例子：escape-uri("http://ddkk.cn/a#c", true())
结果："http%3A%2F%2Fddkk.cn%2Fa#c"
例子：escape-uri("http://ddkk.cn/a#c", false())
结果："http://ddkk.cn/a#c"
例子：escape-uri ("http://ddkk.cn/~bébé", false())
结果："http://ddkk.cn/~b%C3%A9b%C3%A9"
...
fn:substring-before(str1,str2)
返回 str2 在 str1 中出现之前的子字符串

例子：substring-before('12/10','/')
结果：'12'

fn:substring-after(str1,str2)
...
fn:replace(str,pattern,rep)
把指定的模式替换为 rep 参数，并返回结果
...
fn:tokenize(str,pattern)
返回使用 pattern 作为分隔符分隔 str 后的字符串列表

例子：tokenize("XPath is fun", "\s+")
结果：("XPath", "is", "fun")

> [1] Unicode 代码点对照列表 http://www.w3.org/2005/02/xpath-functions/collation/codepoint
...
fn:subsequence((i1,i2,...),start,len)
返回 start 参数指定的位置返回项目序列，序列的长度由 len 参数指定
第一个项目的位置是 1

例子：subsequence(( `$` i1, `$` i2, `$` i3,...), 3)
结果：( `$` i3, ...)
例子：subsequence(( `$` i1, `$` i2, `$` i3, ...), 2, 2)
结果：( `$` i2, `$` i3)
...
fn:zero-or-one(i1,i2,...)
如果参数包含零个或一个项目，则返回参数，否则生成错误

fn:one-or-more(i1,i2,...)
如果参数包含一个或多个项目，则返回参数，否则生成错误

fn:exactly-one(i1,i2,...)
...
**1、** 范例：//book[position()`<=3]的结果：选择前三个book元素↩；

**2、** 范例：//book[last()]的结果：选择最后一个book元素↩；
