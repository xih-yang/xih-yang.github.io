...
- 来源：https://ddkk.com/zhuanlan/java/java8/8.html
- 分类：Java 8 新特性
- 分组：教程目录
经过Java 8 Lambda 表达式 （ 上 ）- 简介 和 Java 8 Lambda 表达式 （ 中 ）- 外部参数 两个章节的学习，想必你已经对 Java 8 中的 Lambda 表达式已经非常熟悉了。
...
在Java 8 之前，在 lambda 表达式出现前，要实现 Runnable 线程，我们需要定义一个实现 Runnable 接口的类来获取 Runnable 对象。
...
public class LambdaTester
{
   public static void main(String args[])
   {
      Runnable r = new Runnable() {
         @Override
         public void run() {
            System.out.println("你好，DDKK.COM 弟弟快看，程序员编程资料站，教程 ");
         }
      };
...
public class LambdaTester
{
   public static void main(String args[])
   {
      Runnable r = () -> System.out.println("你好，DDKK.COM 弟弟快看，程序员编程资料站，教程 ");
      Thread thread = new Thread(r);
      thread.run();
   }
}
