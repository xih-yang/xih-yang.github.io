...
- 来源：https://ddkk.com/zhuanlan/java/java8/13.html
- 分类：Java 8 新特性
- 分组：教程目录
...
empty()
创建一个空 ( empty ) 的 Optional 类的实例

of(T value)
创建一个包含了指定 T 类型的 value 值的 Optional 实例

ofNullable(T value)
如果 value 非 null ，则创建一个包含了指定 T 类型的 value 值的 Optional 实例，否则创建一个空的 Optional 实例
...
```java
[penglei@www.ddkk.com helloworld]$ javac OptionalTester.java && java OptionalTester
