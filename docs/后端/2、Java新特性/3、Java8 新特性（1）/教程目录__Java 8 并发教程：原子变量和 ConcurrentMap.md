...
- 来源：https://ddkk.com/zhuanlan/java/java8new/6.html
- 分类：Java新特性
- 分组：教程目录
...
- 第一部分：[线程和执行器](/zhuanlan/java/java8new/4.html)
- 第二部分：[同步和锁](/zhuanlan/java/java8new/5.html)
- 第三部分：[原子变量和 ConcurrentMap](/zhuanlan/java/java8new/6.html)
...
`java.concurrent.atomic`包包含了许多实用的类，用于执行原子操作。如果你能够在多线程中同时且安全地执行某个操作，而不需要`synchronized`关键字或[上一章](/zhuanlan/java/java8new/5.html)中的锁，那么这个操作就是原子的。

本质上，原子操作严重依赖于比较与交换（CAS），它是由多数现代CPU直接支持的原子指令。这些指令通常比同步块要快。所以在只需要并发修改单个可变变量的情况下，我建议你优先使用原子类，而不是[上一章](/zhuanlan/java/java8new/5.html)展示的锁。
