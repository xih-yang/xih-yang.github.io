# 04、Lua 数据类型
- 来源：https://ddkk.com/zhuanlan/other/lua/4.html
- 分类：Lua 教程
- 分组：教程目录
## Lua 数据类型
...
nil类型表示一种没有任何有效值，它只有一个值 — nil，例如打印一个没有赋值的变量，便会输出一个 nil 值：
...
在Lua 里，table 的创建是通过”构造表达式”来完成，最简单构造表达式是{}，用来创建一个空表。也可以在表里添加一些数据，直接初始化表:
...
在Lua 中，函数是被看作是”第一类值（First-Class Value）”，函数可以存在变量里:
...
在Lua 里，最主要的线程是协同程序（coroutine）。它跟线程（thread）差不多，拥有自己独立的栈、局部变量和指令指针，可以跟其他协同程序共享全局变量和其他大部分东西。
