# 22、Python 教程 - 继承
- 来源：https://ddkk.com/zhuanlan/other/python/4/22.html
- 分类：Python 入门实战
- 分组：教程目录
## 1 继承
...
注释：如果不想向该类添加任何其他属性或方法，请使用 pass 关键字。

现在，Student 类拥有与 Person 类相同的属性和方法。

实例
使用Student 类创建一个对象，然后执行 printname 方法：
...
当添加了 `__init__()` 函数时，子类将不再继承父的 `__init__()` 函数。
...
实例
把名为graduationyear 的属性添加到 Student 类：
