# 02、Python 进阶 - 多进程
- 来源：https://ddkk.com/zhuanlan/other/python/2/2.html
- 分类：Python 进阶
- 分组：教程目录
## 1. 简介
...
在[多线程](/zhuanlan/other/python/2/1.html)一文中，我们说了因为 GIL 的原因，CPython 解释器下的多线程牺牲了并行性，为此 Python 提供了多进程模块 `multiprocessing`，该模块同时提供了本地和远程并发，使用子进程代替线程，可以有效的避免 GIL 带来的影响，能够充分发挥机器上的多核优势，可以实现真正的并行效果，并且它与 `threading` 模块的 API 基本类似，使用起来也比较方便。
...
所有在threading 存在的同步方式，multiprocessing 中都有类似的等价物，如：锁、信号量等。以锁的方式为例，我们来看一个终端打印例子。
...
由Manager() 返回的管理器对象控制一个服务进程，该进程保存 Python 对象并允许其他进程使用代理操作它们。
