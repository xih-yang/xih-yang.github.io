# 13、Python 爬虫 - BeautifulSoup 库
- 来源：https://ddkk.com/zhuanlan/other/python/2/13.html
- 分类：Python 进阶
- 分组：教程目录
...
Tag对象与 HTML 或 XML 原生文档中的 tag 相同，示例如下：
...
Tag有很多方法和属性，这里先看一下它的的两种常用属性：`name` 和 `attributes`。
...
一个tag 可能有很多个属性，先看一它的 `class` 属性，其属性的操作方法与字典相同，示例如下：
...
调用find_all() 方法时，默认会检索当前 tag 的所有子孙节点，通过设置参数 `recursive=False`，可以只搜索 tag 的直接子节点，示例如下：
...
有些tag 属性在搜索不能使用，如 HTML5 中的 `data-*` 属性，示例如下：
