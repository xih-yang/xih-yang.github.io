# 11、Ruby 数据类型
- 来源：https://ddkk.com/zhuanlan/other/ruby/11.html
- 分类：Ruby 教程
- 分组：教程目录
本节我们来看看 Ruby 中的基本数据类型
...
```ruby
#指数算术 
puts 2**(1/4)     # 1 与 4 的商为 0，然后 2 的 0 次方为 1 
puts 16**(1/4.0)  # 1 与 4.0 的商为 0.25（四分之一），然后开四次方根
```
...
Ruby 中可以使用序列 **#{ expr }** 替换任意 Ruby 表达式的值为一个字符串

而且expr 可以是任意的 Ruby 表达式
...
如果想要了解更多有关 Ruby 数组的细节，可以访问我们的 Ruby 数组（Array）
...
如果想要了解更多有关 Ruby 哈希的细节，可以访问我们的 Ruby 哈希（Hash）
...
如果想要了解更多有关 Ruby 范围的细节，可以访问我们的 Ruby 范围 ( Range )
