# 29、Ruby File 类和方法
- 来源：https://ddkk.com/zhuanlan/other/ruby/29.html
- 分类：Ruby 教程
- 分组：教程目录
Ruby 语言中的 **File** 类表示一个连接到普通文件的 *stdio* 对象。 open 方法为普通文件返回该类的一个实例
...
返回path末尾的文件名。如果指定了suffix，则它会从文件名末尾被删除。例如：File.basename("/home/users/bin/ruby.exe") #=> "ruby.exe"
...
17
File::join( item...)
返回一个字符串，由指定的项连接在一起，并使用 File::Separator 进行分隔。例如：File::join("", "home", "usrs", "bin") # => "/home/usrs/bin"
