# 07、Rust 常量
- 来源：https://ddkk.com/zhuanlan/other/rust/7.html
- 分类：Rust 教程
- 分组：教程目录
**常量** 就是那些值不能被改变的变量。一旦我们定义了一个常量，那么就再也没有任何方法可以改变常量的值了。
...
**1、** 定义常量的关键字是const，而定义变量的关键字是let；

**2、** 定义常量时必须指定数据类型，而定义变量时数据类型可以省略，因为会自动推导；

**3、** 常量名的命名规则可变量的命名规则一样，但常量名一般都是**大写字母**；
...
- 常量**只能** 被赋值为 **常量表达式/数学表达式**，不能是 **函数返回值** 或者其它只有在运行时才能确定的值。
