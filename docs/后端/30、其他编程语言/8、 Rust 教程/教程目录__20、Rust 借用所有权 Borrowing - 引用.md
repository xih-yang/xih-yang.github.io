# 20、Rust 借用所有权 Borrowing - 引用
- 来源：https://ddkk.com/zhuanlan/other/rust/20.html
- 分类：Rust 教程
- 分组：教程目录
...
**2、** 函数的参数也必须定义为可变更的，加上**借用Borrowing**或者说引用，也就是必须添加&mut关键字；

**3、** 传递**借用Borrowing**或者说引用也必须声明时**可变更传递**，也就是传递参数时必须添加&mut关键字；
