# 23、Rust 枚举
- 来源：https://ddkk.com/zhuanlan/other/rust/23.html
- 分类：Rust 教程
- 分组：教程目录
...
同时，我们又定义了一个结构体 Person 用于描述一个 **人**。这个 Person 结构体使用 GenderCategory 枚举作为其成员变量 gender 的数据类型
...
fn main() {
   let p1 = Person {
      name:String::from("DDKK.COM 弟弟快看，程序员编程资料站"),
      gender:GenderCategory::Female
   };
   let p2 = Person {
      name:String::from("Yufei"),
...
```sh
Person { name: "DDKK.COM 弟弟快看，程序员编程资料站", gender: Female }
Person { name: "Yufei", gender: Male }
```
...
既然match 语句可以用于比较和判断枚举变量的值，那么对于上面提到的 Option 枚举，match 语句也使用。
