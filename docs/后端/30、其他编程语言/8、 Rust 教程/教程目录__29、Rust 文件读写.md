# 29、Rust 文件读写
- 来源：https://ddkk.com/zhuanlan/other/rust/29.html
- 分类：Rust 教程
- 分组：教程目录
...
结构体File 有相关的成员变量或函数用于表示程序可以对文件进行的某些操作和可用的操作方法。
...
```sh
文件打开成功：File { fd: 3, path: "/Users/penglei/Downloads/guess-game-app/src/data.txt", read: true, write: false }
```
...
```sh
文件创建成功:File { fd: 3, path: "/Users/penglei/Downloads/guess-game-app/src/data.txt", read: false, write: true }
```
...
fn main() {
   ...
   file.write_all("DDKK.COM 弟弟快看，程序员编程资料站".as_bytes()).expect("write failed");
   file.write_all("\nDDKK.COM 弟弟快看，程序员编程资料站".as_bytes()).expect("write failed");
...
**2、** 然后使用read_to_string()函数从文件中读取所有内容并转换为字符串；
...
函数append() 在模块 std::fs::OpenOptions 中定义，它的函数原型为
...
fn main() {
   let mut file = OpenOptions::new().append(true).open("data.txt").expect(
      "cannot open file");
   file.write_all("www.ddkk.com".as_bytes()).expect("write failed");
   file.write_all("\nDDKK.COM 弟弟快看，程序员编程资料站".as_bytes()).expect("write failed");
   file.write_all("\nDDKK.COM 弟弟快看，程序员编程资料站".as_bytes()).expect("write failed");
   println!("数据追加成功");
}
...
打开data.txt 文件，可以看到内容如下

```sh
DDKK.COM 弟弟快看，程序员编程资料站
DDKK.COM 弟弟快看，程序员编程资料站www.ddkk.com
DDKK.COM 弟弟快看，程序员编程资料站
DDKK.COM 弟弟快看，程序员编程资料站
```
...
```sh
[www.ddkk.com]$ ./main.exe data.txt data_new.txt
```

**2、** linux/mac电脑；

```sh
[www.ddkk.com]$ ./main data.txt data_new.txt
```
