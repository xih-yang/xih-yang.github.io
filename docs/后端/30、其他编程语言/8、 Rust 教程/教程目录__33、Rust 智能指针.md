# 33、Rust 智能指针
- 来源：https://ddkk.com/zhuanlan/other/rust/33.html
- 分类：Rust 教程
- 分组：教程目录
...
Box指针也称之 **装箱（ box ）** ，允许我们将数组存储在 **堆 （ heap ）** 上而不是 **栈（ stack ）** 上。

但即使把数据存储在 **堆 （ heap ）** 上，**栈（ stack ）** 仍然包含了指向 **堆**数据的指针。

Box指针没有任何额外的其它开销，因为它仅仅只是把数据存储在 **堆 （ heap ）** 而已。
...
而在Rust 语言中，我们可以把释放内存和资源的操作交给 Drop trait。
