# 12、JVM 实战 - 垃圾回收概述
- 来源：https://ddkk.com/zhuanlan/java/jvm/3/12.html
- 分类：JVM 实战
- 分组：教程目录
## 1. 垃圾回收概述
...
**2、** 除了释放没用的对象，垃圾回收也可以清除内存里的记录碎片碎片整理将所占用的堆内存移到堆的一端，以便JVM将整理出的内存分配给新的对象；

**3、** 随着应用程序所应付的业务越来越庞大、复杂，用户越来越多，没有GC就不能保证应用程序的正常进行而经常造成STW的GC又跟不上实际的需求，所以才会不断地尝试对GC进行优化；
