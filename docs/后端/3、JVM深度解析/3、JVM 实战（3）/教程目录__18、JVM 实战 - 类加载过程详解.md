# 18、JVM 实战 - 类加载过程详解
- 来源：https://ddkk.com/zhuanlan/java/jvm/3/18.html
- 分类：JVM 实战
- 分组：教程目录
## 1. 概述
...
**2、** Java编译器并不会为所有的类都产生`()`初始化方法哪些类在编译为字节码后，字节码文件中将不会包含()方法？；
...
- 全是非静态的字段，不管是否进行了显式赋值，都不会生成``()方法
- 有静态的字段，但是都没有显式的赋值，不会生成``()方法
- 有 显示赋值基本类型或者字符串字面量类型的静态变量(静态代码块也算),将会在初始化阶段赋值,会生成clinit()方法
- 如果静态变量全是使用final修饰,将会在链接阶段的准备环节进行赋值,所以就不会生成``()方法(有例外)
...
虚拟机会保证一个类的 `()` 方法在多线程环境中被正确地加锁、同步，如果多个线程同时去初始化一个类，那么只会有一个线程去执行这类的 `()` 方法， 其他线程都需要阻塞等待，直到活动线程执行 `()`方法完毕 (隐式锁,没有synchronized修饰)

正是因为函数 `()`带锁线程安全的，因此，如果一个在类的 `()`方法中有耗时 很长的操作，就可能造成多个线程阻塞，引发死锁。并且这种死锁是很难发现的， 因为看起来它们并没有可用的锁信息
...
**1、****启动类加载器**加载的类型在整个运行期间是不可能被卸载的(JVM和JSL规范)；

**2、** 被**系统类加载器**和**扩展类加载器**加载的类型在运行期间不太可能被卸载，因为系统类加载器实例或者扩展类的实例基本上在整个运行期间总能直接或者间接的访问的到，其达到unreachable的可能性极小；
