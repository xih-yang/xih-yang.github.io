# 28、JVM 实战 - 分析GC日志
- 来源：https://ddkk.com/zhuanlan/java/jvm/7/28.html
- 分类：JVM 实战
- 分组：教程目录
### 1、GC日志参数
...
- sys：进程在内核态消耗的CPU时间，即在内核执行系统调用或等待系统事件所使用的的CPU时间。
...
- GCViewer可用于可视化Java VM选项-verbose:gc 和.NET生成的数据-Xloggc:。它还计算与垃圾回收相关的性能指标（吞吐量，累积的暂停，最长的暂停等）。当通过更改世代大小或设置初始堆大小来调整特定应用程序的垃圾回收时，此功能非常有用。
