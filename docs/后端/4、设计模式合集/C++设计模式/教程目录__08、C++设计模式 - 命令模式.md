# 08、C++设计模式 - 命令模式
- 来源：https://ddkk.com/zhuanlan/design/c/8.html
- 分类：设计模式
- 分组：教程目录
## 命令模式
...
**2、** 抽取请求并使之成为实现命令接口的具体命令类每个类都必须有一组成员变量来保存请求参数和对于实际接收者对象的引用所有这些变量的数值都必须通过命令构造函数进行初始化；

**3、** 找到担任发送者职责的类在这些类中添加保存命令的成员变量发送者只能通过命令接口与其命令进行交互发送者自身通常并不创建命令对象，而是通过客户端代码获取；
