...
- 来源：https://ddkk.com/zhuanlan/j2ee/spring/1/50.html
- 分类：J2EE框架
- 分组：15.使用JDBC实现数据访问
### 15.2.1 JdbcTemplate
...
另一种替代显式配置的方式是使用component-scanning和注解注入。在这个场景下需要添加[@Repository](https://github.com/Repository)注解（添加这个注解可以被component-scanning扫描到），同时在DataSource的Setter方法上添加[@Autowired](https://github.com/Autowired)注解：
