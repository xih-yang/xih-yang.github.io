# 19.4 FreeMarker
- 来源：https://ddkk.com/zhuanlan/j2ee/spring/1/76.html
- 分类：J2EE框架
- 分组：19. 视图技术
...
Spring提供了一个用于JSP的标签库，其中包含（其中包含）``标签。 此标记主要允许表单从表单支持对象显示值，并显示来自网络或业务层的验证程序的失败验证结果。 Spring还支持FreeMarker中的相同功能，还有其他方便的宏用于生成表单输入元素。
...
> 需要一个“path”参数，它由命令对象的名称组成（除非您在FormController属性中更改它，否则将是“command”），后跟一个句点和该命令上的字段名称 您要绑定到的对象。 也可以使用嵌套字段，如“command.address.street”。 bind macro 假定web.xml中ServletContext参数defaultHtmlEscape指定的默认HTML转义行为。
>
> 称为的宏的可选格式需要第二个参数，并显式指定是否在状态错误消息或值中使用HTML转义。 根据需要设置为true或false。 附加的表单处理宏简化了HTML转义的使用，并且尽可能使用这些宏。 他们将在下一节中进行说明。
...
messageText（根据代码参数从资源束输出一个字符串，退回到默认参数的值）

url(使用应用程序的上下文根前缀相对URL)

formInput（用于收集用户输入的标准输入字段）

formHiddenInput* (用于提交非用户输入的隐藏输入字段)

formPasswordInput* (用于收集密码的标准输入字段。 请注意，不会在此类型的字段中填充任何值)

formTextarea(大文本字段用于收集长，自由格式的文本输入)

formSingleSelect(d下拉框选项允许选择单个所需的值)

formMultiSelect(允许用户选择0个或更多值的选项列表框)

formRadioButtons(一组单选按钮允许从可用选项中进行单次选择)

formCheckboxes(一组允许选择0个或更多值的复选框)
...
- **分隔符：** 多个选项可用作谨慎元素（单选按钮或复选框），用于分隔列表中每个选项的字符序列（即“`
`”）.
