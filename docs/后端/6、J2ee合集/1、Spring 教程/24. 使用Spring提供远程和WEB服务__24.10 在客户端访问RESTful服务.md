...
- 来源：https://ddkk.com/zhuanlan/j2ee/spring/1/94.html
- 分类：J2EE框架
...
POST
postForLocation(String url, Object request, String…​ uriVariables)postForObject(String url, Object request, Class responseType, String…​ uriVariables)
...
每个方法以两种形式使用URI模板参数，作为String可变长度参数或Map``。例如，使用可变长参数如下：
...
String URI变体将模板参数视为String变长参数或者一个Map``。他们还假定URL字符串不被编码且需要编码。例如：

```java
restTemplate.getForObject("http://example.com/hotel list", String.class); 
```

将对 [http://example.com/hotel%20list](http://example.com/hotel%20list) 执行一个 GET 请求。这意味着如果输入的 URL 字符串已被编码，它将被编码两次，即把 `http://example.com/hotel%20list` 再次编码。如果这不是预期效果，则应使用 `java.net.URI` 方法变体，它假设 URL 已经被编码；如果要重复使用单个完整展开的 URI，这种方式通常也更合适。
...
一个HttpMessageConverter的实现，实现从HTTP请求和响应读写表单数据。默认情况下，该转换器读写application/x-www-form-urlencoded媒体类型。表单数据被读取并写入MultiValueMap``。
