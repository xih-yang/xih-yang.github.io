# 3.13 环境抽象
- 来源：https://ddkk.com/zhuanlan/j2ee/spring/1/12.html
- 分类：J2EE框架
- 分组：3. IoC容器
...
当一个或者多个特定的profiles被激活，[@Profile](https://github.com/Profile)注解允许你指定一个有资格的组件来注册。使用我们上面的例子，我们可以按照下面的重写dataSource配置：
...
[@Profile](https://github.com/Profile)可以被用作为创建一个自定义组合注解的元注解。下面的例子定义了一个[@Production](https://github.com/Production)注解，它可以被用作替换[@Profile](https://github.com/Profile)（“production”）的注解。
...
在仅仅包含一个特殊bean的配置类中，[@Profile](https://github.com/Profile)也可以被声明在方法级别：
...
除此之外，profiles也可以通过声明spring.profiles.active属性来激活，这个可以通过在系统环境变量，JVM系统属性，web.xml中的servlet上下文环境参数，甚至JNDI的入口（请参考 3.13.3, “PropertySource abstraction”）。在集成测试中，激活profiles可以通过在Spring-test模块中的[@ActiveProfiles](https://github.com/ActiveProfiles)注解来声明（参见“使用profiles来配置上下文环境”章节）。
...
[@PropertySource](https://github.com/PropertySource)注解对添加一个PropertySource到Spring的环境变量中提供了一个便捷的和声明式的机制。

给出一个名为”app.properties”的文件，它含了testbean.name=myTestBean的键值对，下面的[@Configuration](https://github.com/Configuration)类使用[@PropertySource](https://github.com/PropertySource)的方式来调用testBean.getName()，将会返回”myTestBean”。
...
任何出现在[@PropertySource](https://github.com/PropertySource)中的资源位置占位符都会被注册在环境变量中的资源解析。
