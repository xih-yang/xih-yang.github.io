# 3.14 注册一个加载时编织器
- 来源：https://ddkk.com/zhuanlan/j2ee/spring/1/13.html
- 分类：J2EE框架
- 分组：3. IoC容器
在类被加载进JVM时Spring使用LoadTimeWeaver进行动态转换。

为了使的load-time weaving可用，那么你只需在配置了[@Configuration](https://github.com/Configuration)的类上添加[@EnableLoadTimeWeaving](https://github.com/EnableLoadTimeWeaving)。
