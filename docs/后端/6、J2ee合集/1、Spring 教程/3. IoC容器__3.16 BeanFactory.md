# 3.16 BeanFactory
- 来源：https://ddkk.com/zhuanlan/j2ee/spring/1/15.html
- 分类：J2EE框架
- 分组：3. IoC容器
BeanFactory为Spring的IoC功能提供了底层的基础，但是它仅仅被用于和第三方框架的集成，现在对于大部分的Spring用户来说都是历史了。BeanFactory及其相关的接口，例如：BeanFactoryAware，InitializingBean，DisposableBean，在Spring中仍然有所保留，目的就是为了让大量的第三方框架和Spring集成时保持向后兼容。通常第三方组件不会更加现代的等价物，例如：[@PostConstruct](https://github.com/PostConstruct) 或 [@PreDestroy](https://github.com/PreDestroy)，以便可以与JDK1.4兼容，或避免依赖JSR-250。
