# 6.5 语言参考
- 来源：https://ddkk.com/zhuanlan/j2ee/spring/1/38.html
- 分类：J2EE框架
- 分组：6. Spring表达式语言
### 6.5.1 字符表达式
...
> Note：>/null总是true) 也就是没有一个值会小于什么都没有(x0或者X 备注：需要注意元数据类型会自动装箱成包装类型，因此1 instanceof T(int)结果是false，1 instanceof T(Integer)的结果是true.

每一个符号操作符也可以通过首字母简写的方式标识。这样可以避免表达式所用的符号在当前表达式所在的文档中存在特殊含义而带来的冲突（比如XML文档的``=), eq (==), ne (!=), div (/), mod (%), not (!). 这里不区分大小写。
