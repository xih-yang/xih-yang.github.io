...
- 来源：https://ddkk.com/zhuanlan/j2ee/springmvc/1/21.html
- 分类：J2EE框架
- 分组：6、视图解析
...
- Use a distinct URI for each resource, typically by using a different file extension in the URI. For example, the URI `` requests a PDF representation of the user fred, and `` requests an XML representation.
- Use the same URI for the client to locate the resource, but set the Accept HTTP request header to list the [media types](http://en.wikipedia.org/wiki/Internet_media_type) that it understands. For example, an HTTP request for `` with an Accept header set to application/pdf requests a PDF representation of the user fred, while `` with an Accept header set to text/xml requests an XML representation. This strategy is known as [content negotiation](http://en.wikipedia.org/wiki/Content_negotiation).
...
ofa bean. (See “[Resolving views with the ViewResolver interface](`$`publish-21-5-mvc.html
