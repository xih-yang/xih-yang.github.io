...
- 来源：https://ddkk.com/zhuanlan/j2ee/springboot/7/8.html
- 分类：Spring Boot 3.x 实战
- 分组：实战目录
Springboot集成第三方服务都很方便，比较强大。
...
第一个X - Q1 就有一个 binding key，名字为 orange； X - Q2 就有 2 个 binding key，名字为 black 和 green。*当消息中的 路由键 和 这个 binding key 对应上的时候，那么就知道了该消息去到哪一个队列中。*
...
**1、** 配置Pom包，主要是添加`spring-boot-starter-amqp`的支持；
...
使用queueMessages 同时匹配两个队列，queueMessage 只匹配 “topic.message” 队列
