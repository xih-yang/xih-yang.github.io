...
- 来源：https://ddkk.com/zhuanlan/orm/mybatis/5/2.html
- 分类：ORM框架
- 分组：教程目录
### 全局配置文件说明
...
**1、** 当配置的属性名也存在`:`时（如：`db:username`），此时会有冲突，需要设置自定义的分隔符；
...
NONE: 不做任何反应WARNING: 输出警告日志（‘org.apache.ibatis.session.AutoMappingUnknownColumnBehavior’ 的日志等级必须设置为 WARN）FAILING: 映射失败 (抛出 SqlSessionException)NONE, WARNING, FAILING
NONE
