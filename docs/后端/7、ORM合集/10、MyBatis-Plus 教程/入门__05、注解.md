# 05、注解
- 来源：https://ddkk.com/zhuanlan/orm/mybatisplus/1/5.html
- 分类：ORM框架
- 分组：入门
...
因为MP 底层是 MyBatis，所以 MP 只是帮您注入了常用 CRUD 到 MyBatis 里，注入之前是动态的（根据您的 Entity 字段以及注解变化而变化），但是注入之后是静态的（等于 XML 配置中的内容）。
