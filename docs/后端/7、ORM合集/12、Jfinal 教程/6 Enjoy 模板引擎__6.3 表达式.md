# 6.3 表达式
- 来源：https://ddkk.com/zhuanlan/orm/jfinal/59.html
- 分类：ORM框架
- 分组：6 Enjoy 模板引擎
...
- 算术运算： + - * / % ++ --
- 比较运算： >`>`= `2, null, "abc", 3+6 **(逗号表达式的值为最后一个表达式的值)**
...
key只允许是合法的 java 变量名标识符或者 String 常量值（jfinal 3.4 起将支持 int、long、float、double、boolean、null 等等常量值），**注意：上例中使用了标识符 k1 而非 String 常量值 "k1" 只是为了书写时的便利，与字符串是等价的，并不会对标识符 k1 进行表达式求值。**
...
上述key 为 int 常量，自 jfinal 3.4 版本才开始支持。
