# 17、页面渲染过程
- 来源：https://ddkk.com/zhuanlan/qianduan/vuejs/17.html
- 分类：前端框架
- 分组：Webpack + Vuejs实战
...
因为我们每次打开的都是 ‘[http://localhost/#/](http://localhost/#/)‘, 实际上打开的文件是 ‘[http://localhost/#/index.html](http://localhost/#/index.html)‘
...
这里的``, 就是将来会动态变化的内容。特别类似于 Rails的 layout（模板文件）.
...
上面代码中的 ``, 这个就是第二层的模板。 可以认为该页面的内容，就是在这个位置被渲染出来的。

上面的代码中，还有个 `...`, 这个元素跟`index.html`中的是同一个元素（暂且这么理解）

所有的``中的内容，都会被自动替换。

`` 中的代码则是脚本代码。 至此，整个过程就出来了。

## 原理

Vuejs 就是最典型的Ajax 工作方式: 只渲染部分页面.

浏览器的页面从来不会整体刷新. 所有的页面变化都限定在 `index.html` 中的``代码中，如下图所示:
