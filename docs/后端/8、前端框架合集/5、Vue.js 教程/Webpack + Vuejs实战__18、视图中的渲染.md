# 18、视图中的渲染
- 来源：https://ddkk.com/zhuanlan/qianduan/vuejs/18.html
- 分类：前端框架
- 分组：Webpack + Vuejs实战
前面我们介绍了项目的运行（hello world), 文件夹的结构，以及index.html中的内容是如何一点点的渲染出来的。下面，我们来学习下Vuejs中对于视图的操作。
...
可以看到，上面的代码显示定义了 `message` 这个变量， 然后把它在 ` {{ message }} ` 中显示出来。

打开浏览器 [http://localhost:8080/#/say_hi_from_variable](http://localhost:8080/#/say_hi_from_variable) , 就可以看到下图所示：
