# 19、模板指令directive
- 来源：https://ddkk.com/zhuanlan/qianduan/vuejs/19.html
- 分类：前端框架
- 分组：Webpack + Vuejs实战
...
**4、** Vuejs程序被执行，发现页面中的`Directive`,进行相关的解析；
...
在上面的代码中，可以看到， 通过 `v-bind`, 把``元素的style的值，绑定成了 `'color:' + my_color` 这个表达式。当 `my_color` 的值发生变化的时候， 对应``的颜色就会发生变化。

例如：默认页面打开后， 是绿色的。 如下图所示：

如何知道变量`my_color` 已经绑定到了 `` 上了呢？ 我们在 console中做修改， 让 `app.my_color = "red"`, 就可以看到对应的文字的颜色，变成了红色，如下图所示：
...
上面的代码中， 通过`v-on:click` 的声明，当 被点击(click)后，就会触发 `highlight` 这个方法。
...
使用了`` 来把 变量 `name` 绑定在了 `` 这个输入框上（可以在这里看到`name`的值)

使用了` 你好， {% raw %}{{{% endraw %}name}} !
` 来把变量 `name` 也显示在 页面上。

在初始化中， 使用 `data: { name: '...' }` 的方式， 对变量 `name` 进行了初始化。
