# 28、打包和部署
- 来源：https://ddkk.com/zhuanlan/qianduan/vuejs/28.html
- 分类：前端框架
- 分组：部署和发布
平时我们开发时，是在本地做开发，每次打开的都是 [http://localhost:8080/#/](http://localhost:8080/#/) 。 而真实的项目中，我们肯定要把项目部署到某个地方。
...
**2、** 2把刚才上传的文件夹重命名成:`vue_demo`:；

```js
# mv /opt/app/dist /opt/app/vue_demo 
```

**2、** 3配置nginx,使域名:`vue_demo.siwei.me`指向该位置:；
...
**3、** 1我们需要增加个二级域名:vue_demo.siwei.me；

以dnspod为例, 需要设置这个二级域名的: A记录. IP地址:

保存.

**3、** 2回到命令行,输入ping命令,确认:；
...
说明我的二级域名 `vue_demo.siwei.me` 已经可以正常指向到 我的服务器ip了.

### 4. 部署完成!

打开浏览器, 访问 [http://vue_demo.siwei.me](http://vue_demo.siwei.me) 就可以看到效果了，如下图所示:
