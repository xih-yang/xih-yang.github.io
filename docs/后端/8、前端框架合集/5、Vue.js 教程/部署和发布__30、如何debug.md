# 30、如何debug
- 来源：https://ddkk.com/zhuanlan/qianduan/vuejs/30.html
- 分类：前端框架
- 分组：部署和发布
...
会开启一个”开发服务器”, 这个开发服务器的后台,我们要时刻留意输出.有时候我们把代码写错了, 导致Vuejs无法编译, 前台就会一片空白, 还没有任何出错提示.

上面的错误提示很好理解,说 “编译时出现错误”,也给出了错误的详细位置.

## 看developer tools 提出的日志

无论是chrome, safari 还是firefox, 以及 IE 7+ , 都带有这个工具. 特别好用. 任何时候页面空白了,都要首先看它, 而不是问别人: “页面怎么不动了?”
...
而JSP, PHP, Rails ERB, 则是 “正常框架”, 出了问题可以直接找到最底层.
...
- found in ... `` at ... 这里则是调用栈, 可以看出, 文件是从下调用到最上面的. 问题出在最上面的文件. 但是没有给出错误的行数.
