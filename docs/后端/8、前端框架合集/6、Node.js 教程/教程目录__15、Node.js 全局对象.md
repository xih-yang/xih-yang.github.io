# 15、Node.js 全局对象
- 来源：https://ddkk.com/zhuanlan/qianduan/nodejs/15.html
- 分类：前端框架
- 分组：教程目录
...
**3、** 隐式定义的变量（没有使用var关键字定义的变量)；

定义一个全局变量时，这个变量同时也会成为全局对象的属性，反之亦然

但，在Node.js 中我们不可能在最外层定义变量，因为所有用户代码都是属于当前模块的， 但模块本身不是最外层上下文
...
```javascript
$ node main.js
/Users/penglei/Downloads/curl_mail/node/main.js
```
...
```javascript
$ node main.js
/Users/penglei/Downloads/curl_mail/node
```
...
Node.js **clearTimeout** 全局函数用于停止一个之前通过 setTimeout() 创建的定时器
...
cb
一个函数，setInterval 会在指定时间间隔调用这个函数
...
使用clearInterval 方法清除 setInterval 创建的定时器
...
### console.error() 与 console.trace() 方法
...
$ node main.js
Trace
    at Object.<anonymous> (/Users/luojianguo/Downloads/curl_mail/node/main.js:7:9)
...
Signal
事件当进程接收到信号时就触发。信号列表详见标准的 POSIX 信号名，如 SIGINT、SIGUSR1 等
...
```javascript
$ node main.js Hello ddkk.com
Hello World!
0: /usr/local/Cellar/node/8.7.0/bin/node
1: /Users/luojianguo/Downloads/curl_mail/node/main.js
2: Hello
3: ddkk.com
...
5
getgid()
获取进程的群组标识（参见 getgid(2)）。获取到得时群组的数字 id，而不是名字。注意：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)

6
setgid(id)
设置进程的群组标识（参见 setgid(2)）。可以接收数字 ID 或者群组名。如果指定了群组名，会阻塞等待解析为数字 ID 。注意：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)

7
getuid()
获取进程的用户标识(参见 getuid(2))。这是数字的用户 id，不是用户名。注意：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)

8
setuid(id)
设置进程的用户标识（参见setuid(2)）。接收数字 ID或字符串名字。果指定了群组名，会阻塞等待解析为数字 ID 。注意：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)

9
getgroups()
返回进程的群组 iD 数组。POSIX 系统没有保证一定有，但是 node.js 保证有。注意：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)

10
setgroups(groups)
设置进程的群组 ID。这是授权操作，所有你需要有 root 权限，或者有 CAP_SETGID 能力。注意：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)

11
initgroups(user, extra_group)
读取 /etc/group ，并初始化群组访问列表，使用成员所在的所有群组。这是授权操作，所有你需要有 root 权限，或者有 CAP_SETGID 能力。注意：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)
...
```javascript
$ node main.js
当前目录: /Users/luojianguo/Downloads/curl_mail/node
当前版本: v8.7.0
{ rss: 20881408,
  heapTotal: 7708672,
  heapUsed: 4431888,
