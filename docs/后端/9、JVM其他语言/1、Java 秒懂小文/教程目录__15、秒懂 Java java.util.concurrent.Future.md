...
- 来源：https://ddkk.com/zhuanlan/java/javatm/15.html
- 分类：Java 秒懂小文
- 分组：教程目录
...
好了，现在，我们应该将注意力转移到 [Callable](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/Callable.html) 和 [ExecutorService](/c/penglei/javatm/javatm-basic-executorservice.html) 的使用，因为它们才是最有趣的。
...
> 注意
>
> 对 cancel() 的调用可能会失败。如果调用失败，那么它会返回 false。
> cancel() 方法接受一个布尔值作为参数，该参数用于控制执行此任务的线程是否应该被中断。
