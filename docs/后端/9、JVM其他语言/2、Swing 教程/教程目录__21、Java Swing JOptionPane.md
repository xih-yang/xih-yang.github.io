...
- 来源：https://ddkk.com/zhuanlan/java/swing/21.html
- 分类：Swing 教程
- 分组：教程目录
...
参数parentComponent、message、title、messageType 和 icon 与 showMessageDialog() 方法中的参数的含义相同。其中，只有 parentComponent 和 message 参数是必需的，title 的默认值为“选择一个选项”。messageType 的默认值是 QUESTION_MESSAGE。optionType 参数用于控制在对话框上显示的按钮，可选值如下：
...
String[] str={
     "admin","maxianglin","calcl23456","adminl23"};
...
```java
JButton[] bs={
     new JButton("确定"),new JButton("取消"),new JButton("重置")};
