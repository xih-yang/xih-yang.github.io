# 14、JSP 过滤器
- 来源：https://ddkk.com/zhuanlan/java/jsp/14.html
- 分类：JSP 教程
- 分组：教程目录
## JSP 过滤器
...
过滤器被定义，然后映射成一个URL或JSP文件名，与servlet被定义然后映射的方式差不多。在部署描述文件web.xml中，使用 标签来进行过滤器映射：
...
在web.xml中 元素的映射顺序决定了容器应用这些过滤器的顺序。要反转应用的顺序，您只需要反转web.xml中 元素的定义顺序就行了。
