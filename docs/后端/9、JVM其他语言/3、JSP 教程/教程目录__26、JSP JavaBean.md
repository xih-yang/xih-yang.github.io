# 26、JSP JavaBean
- 来源：https://ddkk.com/zhuanlan/java/jsp/26.html
- 分类：JSP 教程
- 分组：教程目录
## JSP JavaBean
...
其中，根据具体情况，scope的值可以是page，request，session或application。id值可任意只要不和同一JSP文件中其它 中id值一样就行了。
...
在标签主体中使用 标签来调用getter方法，使用 标签来调用setter方法，语法格式如下：
