# 15、Scala 教程： 数组
- 来源：https://ddkk.com/zhuanlan/java/scala/15.html
- 分类：Scala 教程
- 分组：教程目录
...
```java
z(0) = "DDKK.COM 弟弟快看，程序员编程资料站"; z(1) = "百度"; z(4/2) = "腾讯"
```
...
```java
var z = Array("DDKK.COM 弟弟快看，程序员编程资料站", "百度", "腾讯")
```
...
返回指定长度数组，每个数组元素为指定函数的返回值。以上范例数组初始值为 0，长度为 3，计算函数为a=>a+1：scala> Array.iterate(0,3)(a=>a+1)
...
返回指定长度数组，每个数组元素为指定函数的返回值，默认从 0 开始。以上范例返回 3 个元素：scala> Array.tabulate(3)(a => a + 5)
