...
- 来源：https://ddkk.com/zhuanlan/tiku/spring.html
- 分类：私域粉丝-面试题
- 分组：面试题目录
## 一、Spring概述
...
**5、** 请求处理事件（RequestHandledEvent）：在Web应用中，当一个http请求（request）结束触发该事件如果一个bean实现了ApplicationListener接口，当一个ApplicationEvent被发布以后，bean会自动被通知；
...
对于IoC 来说，最重要的就是容器。容器管理着 Bean 的生命周期，控制着 Bean 的依赖注入。
...
调用getBean 的时候，从 BeanDefinition 所属的 Map 里，拿出 Class 对象进行实例化，同时，如果有依赖关系，将递归调用 getBean 方法 —— 完成依赖注入。
...
**1、** 配置theHibernateSessionFactory；
...
这就是博主分享给大家的Spring常见的77道面试题及答案（2021版）
